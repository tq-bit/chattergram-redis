...
import { PublicFileSchemaType } from '../api/schemata/index';

// FIXME: Older files are not getting deleted when the container restarts

export interface AppFileUpload extends PublicFileSchemaType {
  path: string;
}
...
export default class Uploader {
  ...
  constructor(filepath?: string) {
    this.filepath =
      filepath ||
      path.join(
        process.env.APP_WORKDIR ? process.env.APP_WORKDIR : './',
        '/tmp',
      );
    this.fields = {};
    this.files = [];
  ...
  private deleteFileAfterTimeout(
    path: PathLike,
    timeout: number,
  ): Promise<boolean> {
    return new Promise((resolve, reject) => {
